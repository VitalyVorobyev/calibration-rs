...
fn scheimpflug_optimization_synthetic() {
    ...
    for (view_idx, (points_3d, points_2d)) in views.iter().enumerate() {
        ...
        let pose_id = ir.add_param_block(
            &pose_key,
            7,
            ManifoldKind::SE3,
            FixedMask::all_free(),
    ...
    println!("Intrinsics errors: fx={:.2e}, fy={:.2e}, cx={:.2e}, cy={:.2e}",
    ...
    println!("Distortion errors: k1={:.2e}, k2={:.2e}, p1={:.2e}, p2={:.2e}",
    ...
    println!("Scheimpflug errors: tilt_x={:.2e}, tilt_y={:.2e}",
             tilt_x_error, tilt_y_error);

    assert!(tilt_x_error < 0.001, "tilt_x error too large: {}", tilt_x_error);
    assert!(tilt_y_error < 0.001, "tilt_y error too large: {}", tilt_y_error);
