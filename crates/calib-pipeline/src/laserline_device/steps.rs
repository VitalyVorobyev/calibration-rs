...
use calib_linear::laserline::{LaserlinePlaneSolver, LaserlineView as LinearLaserlineView};
use calib_linear::prelude::*;
use calib_optim::{compute_laserline_stats, optimize_laserline, LaserlineParams, LaserlineStats};
...
fn planar_dataset_from_input(input: &[calib_optim::LaserlineView]) -> Result<PlanarDataset> {
...
fn estimate_poses(
    input: &[calib_optim::LaserlineView],
    intrinsics: &FxFyCxCySkew<calib_core::Real>,
) -> Result<Vec<Iso3>> {
...
fn linear_plane_init<Sm>(
    ...
    camera: &Camera<
        calib_core::Real,
        Pinhole,
        calib_core::BrownConrady5<calib_core::Real>,
        Sm,
        FxFyCxCySkew<calib_core::Real>,
    >,
    poses: &[Iso3],
) -> Result<(calib_optim::LaserPlane, f64)>
where
    Sm: SensorModel<calib_core::Real>,
{
    ...
    let plane = calib_optim::LaserPlane::new(estimate.normal.into_inner(), estimate.distance);
